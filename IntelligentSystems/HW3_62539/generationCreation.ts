...
import { getPopulationLimit } from "./limits";
import { objectLimit } from "./main";
 ...
 */
export function createFromChildren(
  ...
  while (
    i < parents.length &&
    population.length < getPopulationLimit(objectLimit)
  ) {
 ...
 */
export function createFromMixing(population: Population, children: Population) {
  population.push(...children);

  population.sort((a, b) => b.fitness - a.fitness);

  const populationLimit = getPopulationLimit(objectLimit);
  const halfSize = Math.floor(populationLimit / 2);
  const remaining = population.slice(halfSize);
  ...
  const remainingSize = populationLimit - halfSize;
  const remainingToAdd = remaining.slice(0, remainingSize);

  remainingToAdd.sort((a, b) => b.fitness - a.fitness);
 ...
 * @param children - The offspring generated through crossover and mutation.
 */
export function createFromElitism(
  population: Population,
  children: Population
) {
  const populationLimit = getPopulationLimit(objectLimit);
